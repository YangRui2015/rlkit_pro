...
class TensorBoardPrinter(object):
    """
    ...
    def __init__(self, dir):
        ...
        path = osp.join(osp.abspath(dir), prefix + '_' + time_stamp)
    ...
    def write_tabular(self, tabulars):
        def summary_val(k, v):
            kwargs = {'tag': k, 'simple_value': float(v)}
            return self.tf.Summary.Value(**kwargs)
        summary = self.tf.Summary(value=[summary_val(k, v) for k, v in tabulars])
...
class Logger(object):
    ...
    def _set_tb(self, dir):
        self.use_tb = True
        self.tb_writer = TensorBoardPrinter(dir)
    ...
    def dump_tabular(self, *args, **kwargs):
        ...
        if len(self._tabular) > 0:
            ...
            if self.use_tb:
                self.tb_writer.write_tabular(self._tabular)
